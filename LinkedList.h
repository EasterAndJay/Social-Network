...
public:
	...
	template <class A>
	friend std::ostream & operator<<(std::ostream & os, const LinkedList<A> & list);
private:
	Node<T> * head;
	Node<T> * tail;
	int length;
...
template <class T>
void LinkedList<T>::set(int pos, const T & data) {
...
template <class A>
std::ostream & operator<<(std::ostream & os, const LinkedList<A> & list) {
